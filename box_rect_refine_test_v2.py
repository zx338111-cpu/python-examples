...
# -*- coding: utf-8 -*-
"""
box_rect_refine_test_v2.py

改进点：
1. 在 ROI 的四周直接清零，避免把 ROI 边界当成矩形
2. 过滤“贴边轮廓”
3. 过滤“几乎等于 ROI 大小”的伪矩形
4. 继续保留 YOLO 粗检测 + OpenCV 矩形精提 + 调试图保存
...
class BoxRectRefineTestV2:
    ...
    def refine_rect(self, frame: np.ndarray, det: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        # 关键修复 1：直接把 ROI 四周边界清零，防止 ROI 边界成为最大矩形
        edges[:m, :] = 0
        edges[-m:, :] = 0
        edges[:, :m] = 0
        edges[:, -m:] = 0
        ...
        for mode_name, bin_img in [("edges", edges), ("thresh", th)]:
            ...
            for c in contours:
                ...
                pts = c.reshape(-1, 2)
                # 关键修复 2：任何贴 ROI 边界的轮廓都丢掉
                if (
                    pts[:, 0].min() <= m
                    or pts[:, 1].min() <= m
                    or pts[:, 0].max() >= roi_w - 1 - m
                    or pts[:, 1].max() >= roi_h - 1 - m
                ):
                    continue
                ...
                # 关键修复 3：如果候选矩形几乎跟 ROI 一样大，也丢掉
                if rw >= roi_w - 2 * m or rh >= roi_h - 2 * m:
                    continue
    ...
    def run(self) -> int:
        ...
        print(f"🎯 开始检测+矩形精提 V2，camera={self.camera_name}, class={self.class_name}, retries={self.retries}")
...
def build_argparser():
    ap = argparse.ArgumentParser(description="G2 右/左手相机 + YOLO + 矩形精提测试脚本 V2")
    ...
    ap.add_argument("--roi-pad", type=int, default=20, help="YOLO bbox 向外扩的像素，默认 20")
    ap.add_argument("--border-margin", type=int, default=8, help="ROI 边缘屏蔽宽度，默认 8")
    return ap

...
def move_waist_to_named_joint_point(robot, point_name, step_rad=0.10, pause_s=0.00):
    ...
    print(
        ...
        f"segments={segments}, "
        f"step_rad={step_rad}, pause_s={pause_s}"
    )
...
def goto_named_joint_pose(robot, point_name, arm_duration_s=6.0, move_waist_first=True, waist_step_rad=0.10):
    point = robot.get_point(point_name)
    if point is None:
        raise RuntimeError(f"未找到点位: {point_name}")
    ...
    if move_waist_first:
        move_waist_to_named_joint_point(robot, point_name, step_rad=waist_step_rad, pause_s=0.00)

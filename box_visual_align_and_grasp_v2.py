...
# -*- coding: utf-8 -*-
"""
box_visual_align_and_grasp_v2.py

流程：
1. 到拍照位：view_waist + view_pose_calib
2. 右手相机 YOLO 检测箱子
3. 用拍照位模板误差 + 局部逆 Jacobian，算出双臂整体修正量 dx/dy/dz
4. 到抓取位：grasp_waist + grasp_pose
5. 双臂一起叠加整体修正
6. 右臂再叠加固定 bias，补右臂相对抓取几何误差
7. 可选闭夹爪抓取
...
class BoxVisualAlignAndGraspV2:
    def __init__(
        self,
        model_path: str,
        class_name: str = "box",
        ...
        self.RIGHT_ARM_BIAS_DX = 0.0
        self.RIGHT_ARM_BIAS_DY = 0.090
        self.RIGHT_ARM_BIAS_DZ = 0.0
    ...
    def draw_detection(self, frame: np.ndarray, det: Dict[str, Any], dx: float, dy: float, dz: float) -> np.ndarray:
        ...
        cv2.putText(
            img,
            f"global delta: dx={dx:+.4f} dy={dy:+.4f} dz={dz:+.4f}",
            (20, 65),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.65,
            (255, 0, 255),
    ...
    def run(self):
        ...
        print(f"计算整体修正量: dx={dx:+.4f} m, dy={dy:+.4f} m, dz={dz:+.4f} m")
...
def build_argparser():
    ap = argparse.ArgumentParser(description="拍照位 YOLO 对齐 -> 修正抓取位 -> 抓取（V2，含右臂 bias）")
